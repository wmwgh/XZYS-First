...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (model.isSender) {
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", XZYS_PJ_URL, self.dataDic[@"member_picture"]]] placeholderImage:[UIImage imageNamed:@"hx01.jpg"]];
        _nameLabel.text = self.dataDic[@"nickname"];
    } else {
        if (model.avatarURLPath) {
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholderImage:model.avatarImage];
        } else {
            self.avatarView.image = [UIImage imageNamed:@"hx01.jpg"];
        }
        _nameLabel.text = @"店小二";
    }
